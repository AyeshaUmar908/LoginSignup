...
public class signup extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        login = FirebaseDatabase.getInstance().getReference().child("users");
            ...
            public void onClick(View v) {
                ...
                if (!TextUtils.isEmpty(userEmail) && !TextUtils.isEmpty(userPass) && !TextUtils.isEmpty(userConfPass) && !TextUtils.isEmpty(userName) && !TextUtils.isEmpty(userTelephone))
                {
                    if (validateEmail(userEmail)== true)
                    {
                            ...
                            auth.createUserWithEmailAndPassword(userEmail,userPass).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                                ...
                                public void onComplete(@NonNull Task<AuthResult> task) {
                                    if (task.isSuccessful())
                                    {
                                        ...
                                        login.child(uid).setValue(newuser);

                                        progress.dismiss();
                                        Toast.makeText(signup.this,"You have been successfuly registered, please login to continue!",Toast.LENGTH_SHORT).show();
                                    ...
                                    {
                                        progress.dismiss();
                                        Toast.makeText(signup.this,task.getException().getMessage()+" Try a different Email",Toast.LENGTH_SHORT).show();
                                    }
                                }
                            });
                        }
                        ...
                        {
                            progress.dismiss();
                            Toast.makeText(signup.this,"Passwords you entered do not match, Please try again!",Toast.LENGTH_SHORT).show();
                        }

                    }
                    else
                    {
                        Toast.makeText(signup.this,"Email Invalid, Please enter a valid Email!",Toast.LENGTH_SHORT).show();
                    }
                }
                else
                {
                    Toast.makeText(signup.this,"You cannot have one or more empty fields!",Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
